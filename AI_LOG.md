...
- **[Limitation — not fixed]** I could not visually verify the UI in a browser: the Chrome extension wasn't connected in this environment. The page and both static assets are confirmed served with correct content types, and the JS passes `node --check`, but no human or automated eye has seen it render. Stated as-is rather than implied to be verified.
